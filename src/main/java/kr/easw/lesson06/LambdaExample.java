...
public class LambdaExample {
    ...
    private static int lambdaResult(int base, int delta) {
        BiFunction<Integer, Integer, Integer> calculateAdjust = (first, second) -> {
            return first + second;
        };
        Consumer<Integer> negativeDelta = first -> System.out.println(-first);
        int adjusted = calculateAdjust.apply(base, delta);
        negativeDelta.accept(delta);
        return adjusted;
    }
